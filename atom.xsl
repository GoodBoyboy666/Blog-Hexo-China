...
<xsl:stylesheet version="3.0"
    ...
    <xsl:template match="/">
        ...
        <html lang="en">
            <head>
                ...
                <link rel="stylesheet" crossorigin="" href="/assets/app-D0ioTNtu.css" />
            </head>
            <body>
              ...
              tabindex="-1">
                  ...
                  class="flex flex-grow flex-row w-full py-3 px-4 gap-x-1 rounded-medium items-center text-primary-600 bg-primary-50 dark:bg-primary-50/50">
                    ...
                    class="h-full flex-grow min-h-10 ms-2 flex flex-col box-border text-inherit justify-center items-center">
                                        ...
                                        <div class="mt-5 mb-5 px-4 sm:px-10">
                                            <xsl:apply-templates select="atom:feed/atom:entry"></xsl:apply-templates>
                                        </div>
                                    </div>
                                    <div class="flex justify-center">
                                        <p class="text-gray-800 mb-10 break-all">Copyright © 2025 GoodBoyboy All Rights Reserved.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <xsl:template match="atom:entry">
        <div
            class="flex flex-col relative overflow-hidden h-auto text-foreground box-border bg-content1 outline-solid outline-transparent data-[focus-visible=true]:z-10 data-[focus-visible=true]:outline-2 data-[focus-visible=true]:outline-focus data-[focus-visible=true]:outline-offset-2 shadow-medium rounded-large transition-transform-background motion-reduce:transition-none mb-5 max-w-2xl min-w-0" tabindex="-1">
            <div class="p-3 z-10 w-full justify-start items-center shrink-0 overflow-inherit color-inherit subpixel-antialiased rounded-t-large flex gap-3">
                <div class="flex-col">
                    <a class="relative inline-flex items-center tap-highlight-transparent outline-solid outline-transparent data-[focus-visible=true]:z-10 data-[focus-visible=true]:outline-2 data-[focus-visible=true]:outline-focus data-[focus-visible=true]:outline-offset-2 text-primary no-underline hover:opacity-hover active:opacity-disabled transition-opacity text-[20px] break-all"
                        href="#" tabindex="0" data-react-aria-pressable="true" role="link">
                        <xsl:value-of select="atom:title"></xsl:value-of>
                    </a>
                    <div class="flex-row text-[15px]">
                        <span class="publish-date">
                            <xsl:value-of select="atom:published"></xsl:value-of>
                        </span>
                        <span> | </span>
                        <span>
                            <xsl:for-each select="atom:category[not(preceding-sibling::atom:category/@term = @term)]">
                                <span>
            ...
            <hr class="shrink-0 bg-divider border-none w-full h-divider" role="separator" />
            <div class="p-3 h-auto flex w-full items-center overflow-hidden color-inherit subpixel-antialiased rounded-b-large">
                <a class="relative inline-flex items-center tap-highlight-transparent outline-solid outline-transparent data-[focus-visible=true]:z-10 data-[focus-visible=true]:outline-2 data-[focus-visible=true]:outline-focus data-[focus-visible=true]:outline-offset-2 text-medium text-primary no-underline hover:opacity-hover active:opacity-disabled transition-opacity break-all" href="#" target="_blank" rel="noopener noreferrer" tabindex="0" data-react-aria-pressable="true" role="link">查看原文
